...
function renderBattleDice(){const draw=(id,grid,clickable)=>{const el=document.getElementById(id);if(!el)return;const cells=Array.from({length:15},(_,i)=>grid?.[i]||randomBattleDie('empty'));if(grid){while(grid.length<15)grid.push(cells[grid.length]);}el.innerHTML=cells.map((die,i)=>`<div class="slot"><div onclick="${clickable?`battleSelect(${i})`:''}">${battleDiceHTML(die,clickable&&appState.selected===i)}</div></div>`).join('')};draw('myDiceGrid',appState.diceGrid,true);draw('opponentDiceGrid',appState.battle?.opponentDice||[],false);}
