...
const D_ART={blue:'🌀',cyan:'❄️',red:'🔥',green:'☠️',yellow:'☀️',pink:'✿',purple:'✦',orange:'⚡',black:'🌑',white:'✧',silver:'🛡️',gold:'💰',aqua:'🌊',violet:'🕳️',lime:'💚',navy:'💣',coral:'🪸',mint:'🌿',teal:'🔷',magenta:'💜',brown:'🪨',crimson:'❤️'};
...
function diceHTML(type,lvl=1,rank=1){const d=D[type]||D.blue;return `<div class="dice ${d.cls}" data-dice-type="${esc(type)}"><span class="dieArt">${D_ART[type]||D_ART.blue}</span><small>Lv.${Math.max(1,Math.min(50,Number(lvl)||1))}</small></div>`;}
...
function selectTreeNode(id){const n=Number(id);if(!treeNode(n))return;appState.selectedTreeNode=n;drawTree();ensureBackButton('tree');}
async function upgradeTreeDice(type){try{const r=await api('/api/tree/upgrade',{method:'POST',body:JSON.stringify({type})});applyServerResponse(r);drawTree();ensureBackButton('tree');updateRes();toast(r.upgrade?.label||`${D[type]?.name||'업그레이드'} 업그레이드 완료`);}catch(e){toast(e.message)}}
async function buyTree(id){ensureTreeState();const node=treeNode(id);if(!node)return;if(treeUnlocked(id)){appState.selectedTreeNode=id;drawTree();ensureBackButton('tree');return;}if(!treeIsNext(id)){toast('먼저 연결된 앞의 노드를 해금하세요');return;}try{const r=await api('/api/tree/unlock',{method:'POST',body:JSON.stringify({nodeId:id})});applyServerResponse(r);appState.selectedTreeNode=id;drawTree();ensureBackButton('tree');updateRes();toast(`${node.label} 해금 완료 · 🎲 -8`);}catch(e){toast(e.message)}}
