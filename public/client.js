...
function ensureBattleGrid(grid,prefix='fill'){const arr=Array.isArray(grid)?grid:[];for(let i=0;i<15;i++){if(!arr[i]||!D[arr[i].type])arr[i]=randomBattleDie(prefix+'-'+i);}arr.length=15;return arr;}
function renderBattleDice(){const draw=(id,grid,clickable,prefix)=>{const el=document.getElementById(id);if(!el)return;const safe=ensureBattleGrid(grid,prefix);el.innerHTML=safe.map((die,i)=>`<div class="slot"><div class="battleDieHolder" onclick="${clickable?`battleSelect(${i})`:''}">${battleDiceHTML(die,clickable&&appState.selected===i)}</div></div>`).join('')};appState.diceGrid=ensureBattleGrid(appState.diceGrid,'me');if(appState.battle)appState.battle.opponentDice=ensureBattleGrid(appState.battle.opponentDice,'opp');draw('myDiceGrid',appState.diceGrid,true,'me');draw('opponentDiceGrid',appState.battle?.opponentDice||[],false,'opp');}
function summonBattleDice(){if(appState.sp<50)return toast('SP가 부족합니다.');appState.sp-=50;appState.diceGrid=ensureBattleGrid(appState.diceGrid,'me');let idx=0,min=99;appState.diceGrid.forEach((d,i)=>{if((d?.rank||1)<min){min=d.rank;idx=i;}});const spawned=randomBattleDie('summon');appState.diceGrid[idx]=spawned;appState.selected=null;renderBattleDice();sendBattle({type:'summon',slot:idx,die:spawned});updateBattleHUD();toast(`${D[spawned.type]?.name||'주사위'} 1성 소환`);}
...
function setupBattle(){appState.sp=100;appState.battleDiceUpgrades={};appState.kills=0;appState.gameSec=0;appState.speed=1;appState.selected=null;appState.coopEnded=false;appState.killGoal=5000;appState.nextBossKill=150;appState.bossIndex=0;appState.myBossCount=0;appState.oppBossCount=0;appState.battleUpgradeLevel=0;appState.battleUpgradeDamage=0;appState.enemies=[];appState.oppEnemies=[];appState.sharedEnemies=[];appState.diceGrid=ensureBattleGrid(makeFullGrid(appState.deck||['blue'],'me'),'me');appState.battle.myBase=150;appState.battle.opponentBase=150;appState.battle.sharedBase=150;appState.battle.opponentSpeed=1;appState.battle.opponentDice=appState.opponentType==='ai'?ensureBattleGrid(makeFullGrid(appState.deck||['blue'],'ai'),'ai'):ensureBattleGrid([],'opp');last=0;lastAttack=0;lastOpponentAttack=0;appState._atk={};renderBattleDice();updateBattleHUD();for(let i=0;i<6;i++){setTimeout(()=>addEnemyFor('me'),i*260);setTimeout(()=>addEnemyFor(appState.battle.mode==='coop'?'shared':'opp'),i*260);}if(appState.opponentType==='player'&&appState.matchId)sendBattle({type:'battle_join',matchId:appState.matchId,grid:appState.diceGrid,speed:appState.speed,mode:appState.battle.mode});window.rd2BattleRaf=requestAnimationFrame(battleLoop);}
